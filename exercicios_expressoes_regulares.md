[material-estudante-expressoes-regulares-vfinal.md](https://github.com/user-attachments/files/32174198/material-estudante-expressoes-regulares-vfinal.md)
...
---
# Respostas — Expressões Regulares
...
## Desafio final — Código de matrícula
...
`(CCO|ESW|SIS)` são os cursos.

`(2024|2025|2026|2027|2028|2029)` são os anos permitidos.

`[0-9]{4}` são os 4 números da matrícula.

`(M|T|N)` são os turnos.

`^` e `$` servem para não deixar caracteres extras.
...
1. `CCO-2030-1234-M` — ano inválido.
2. `ESW-2026-123-N` — faltou um número.

### Perguntas

1. `(CCO|ESW|SIS)`

2. Foram colocados apenas os anos de 2024 até 2029.

3. `{4}` pede exatamente 4 números. Já `+` aceita um ou mais.

4. `^` marca o começo e `$` o final da entrada.

5. Não. Os testes mostram casos que devem passar e casos que devem ser rejeitados.

## Desafio extra — DFA

- Os estados mostram o caminho da matrícula, começando pelo curso e passando pelo ano, número e turno.
- As escolhas aparecem nos cursos, nos anos permitidos e nos turnos.
- O estado final é o único estado de aceitação.
- Se tiver algum caractere errado ou algum caractere depois do turno, vai para o estado sumidouro.

## Regex Learn — testes

1. `CCO-2024-5678-T` — aceita
2. `SIS-2029-4321-N` — aceita
3. `ESW-2026-123-N` — rejeita
4. `CCO-2027-1234-MX` — rejeita
...
3. Os três representam linguagens regulares. A ER pode virar um NFA e depois um DFA.

4. A Regex teórica tem os operadores básicos. Os programas podem ter outros recursos.

5. Porque o autômato finito tem poucos estados e não consegue guardar uma quantidade ilimitada.

6. Porque teria que contar os `a` e depois comparar com os `b`.

7. Nas linguagens livres de contexto pode ser usada uma pilha, que permite guardar mais informações.

<img width="1504" height="904" alt="image" src="https://github.com/user-attachments/assets/45d30b73-626e-4958-a211-2916648bfb78" />
